...
import Sidebar from '$lib/components/layout/Sidebar.svelte';
  ...
  $effect(() => {
    ...
    // Beep when an agent completes a run
    if (latest.event_type === 'run.completed' || latest.type === 'run.completed') {
      ambientStore.onAgentCompleted(latest.agent_name ?? 'Agent');
    }
  });
  ...
  onMount(() => {
    ...
    initializeAuth().then(async () => {
      // Auth bypassed — mark onboarding complete and proceed
    ...
    // Initialize ambient audio (lofi/sonic/beep)
    ambientStore.init();

    return () => {
      stopPolling?.();
      activityStore.unsubscribe();
      ambientStore.destroy();
    };
  });
...
<button
  ...
  onclick={() => ambientStore.toggle()}
  aria-label={ambientStore.muted ? 'Unmute ambient audio' : 'Mute ambient audio'}
  title={ambientStore.muted ? 'Unmute' : `${ambientStore.currentTrack === 'sonic' ? '⚡ Active' : '🎵 Lofi'} — Click to mute`}
>
  {ambientStore.muted ? '🔇' : ambientStore.currentTrack === 'sonic' ? '⚡' : '🎵'}
</button>
