...
// HTTP API client for the NULLHACK backend with mock fallback
...
let useMock = false; // Auth bypassed — backend is always available
...
// ── Auth Gate ─────────────────────────────────────────────────────────────────
// Auth bypassed — gate is always open.

let _authResolve: (() => void) | null = null;
const _authPromise: Promise<void> = Promise.resolve();
...
async function _doInitializeAuth(): Promise<void> {
  // Auth bypassed — just probe health and open the gate.
  // Backend no longer requires JWT tokens.
  try {
...
async function request<T>(path: string, options: RequestInit = {}): Promise<T> {
  // Auth bypassed, mock disabled — always hit the real backend.
...
export const sessions = {
  ...
  get: async (id: string): Promise<Session> => {
    const data = await request<{ session: Session } | Session>(`/sessions/${id}`);
...
export const messages = {
  list: async (sessionId: string) => {
    try {
      const url = `${BASE_URL}${API_PREFIX}/sessions/${sessionId}/transcript`;
      ...
      console.log("[transcript] bytes", text.length);
      const data = JSON.parse(text) as { messages: import("./types").Message[] };
      console.log("[transcript] messages", data.messages?.length ?? 0);
...
export const issues = {
  ...
  get: async (id: string): Promise<Issue> => {
    const data = await request<{ issue: Issue } | Issue>(`/issues/${id}`);
    return (data as { issue: Issue }).issue ?? (data as Issue);
  },
...
export const goals = {
  list: async (projectId: string): Promise<GoalTreeNode[]> => {
    ...
    const map = new Map(flat.map((g) => [g.id, g]));
...
export const documents = {
  list: async (
    ...
    const qs = workspaceId
      ...
      : "";
    // API returns { files: [{name, path, type, size}], directory } (filesystem listing)
    // We need to recursively fetch subdirectories to get actual files
    ...
    // If the API already returns documents/tree format, use it directly
    if (data.documents) {
      return { documents: data.documents, tree: data.tree ?? [] };
    }

    // Transform filesystem listing into Document objects
    const files = data.files ?? [];
    const docs: Document[] = [];
    const tree: DocumentTreeNode[] = [];

    // Fetch contents of each directory to find actual files
    for (const entry of files) {
      if (entry.type === 'directory') {
        try {
          const subData = await request<{
            files?: { name: string; path: string; type: string; size: number }[];
          }>(`/documents/${entry.name}${qs}`);
          ...
          for (const sf of subFiles) {
            if (sf.type === 'file') {
              ...
              const doc: Document = {
                path: fullPath,
                title: sf.name.replace(/\.\w+$/, '').replace(/[-_]/g, ' '),
                format: sf.name.endsWith('.md') ? 'markdown' : 'text',
                size: sf.size,
                created_at: '',
                updated_at: '',
              };
              docs.push(doc);
              children.push({ name: sf.name, path: fullPath, type: 'file', children: [] });
            }
          }
          tree.push({ name: entry.name, path: entry.name, type: 'directory', children });
        } catch { /* skip broken dirs */ }
      } else {
        const doc: Document = {
          path: entry.path,
          title: entry.name.replace(/\.\w+$/, '').replace(/[-_]/g, ' '),
          format: entry.name.endsWith('.md') ? 'markdown' : 'text',
          size: entry.size,
          created_at: '',
          updated_at: '',
        };
        docs.push(doc);
        tree.push({ name: entry.name, path: entry.path, type: 'file', children: [] });
      }
    }

    return { documents: docs, tree };
  },
