...
<script lang="ts">
  ...
  import type { CanopyAgent, AgentStatus, AgentLifecycleAction, Session, ActivityEvent } from '$api/types';
  ...
  // Live data fetched when agent is selected
  let agentSessions = $state<Session[]>([]);
  let agentActivity = $state<ActivityEvent[]>([]);
  let liveTokens = $state(0);
  let liveCost = $state(0);
  let currentTask = $state<string | null>(null);

  // Fetch live data when agent changes
  $effect(() => {
    if (!agent) return;
    const id = agent.id;
    // Fetch sessions for this agent
    fetch(`http://127.0.0.1:9089/api/v1/sessions?workspace_id=${agent.workspace_id}`)
      .then(r => r.json())
      .then((d: { sessions: Session[] }) => {
        const mine = (d.sessions ?? []).filter(s => s.agent_id === id);
        agentSessions = mine;
        // Calculate totals
        liveTokens = mine.reduce((sum, s) => sum + (s.tokens_input ?? 0) + (s.tokens_output ?? 0), 0);
        liveCost = mine.reduce((sum, s) => sum + (s.cost_cents ?? 0), 0);
        // Current task = latest active session
        const active = mine.find(s => s.status === 'active');
        currentTask = active ? `Session ${active.id.slice(0, 8)}... (${active.model})` : null;
      })
      .catch(() => {});
    // Fetch activity for this agent
    fetch(`http://127.0.0.1:9089/api/v1/activity?agent_id=${id}`)
      .then(r => r.json())
      .then((d: { events: ActivityEvent[] } | ActivityEvent[]) => {
        const events = Array.isArray(d) ? d : (d.events ?? []);
        agentActivity = events.filter(e => e.agent_id === id).slice(0, 8);
      })
      .catch(() => {});
  });
...
{#if agent}
  ...
  >
    ...
    <div class="odp-section">
      ...
      {#if currentTask}
        <p class="odp-task">{currentTask}</p>
      {:else if agentSessions.length > 0}
        <p class="odp-task">{agentSessions.length} session{agentSessions.length !== 1 ? 's' : ''} completed</p>
      {:else}
        <p class="odp-task odp-task--empty">No active task</p>
      {/if}
    </div>

    <!-- Quick stats (live from sessions) -->
    <div class="odp-section">
      <div class="odp-section-label">Today</div>
      <div class="odp-stats">
        <div class="odp-stat">
          <span class="odp-stat-label">Tokens</span>
          <span class="odp-stat-value">{liveTokens >= 1000 ? `${(liveTokens / 1000).toFixed(0)}K` : String(liveTokens)}</span>
        </div>
        <div class="odp-stat-divider"></div>
        <div class="odp-stat">
          <span class="odp-stat-label">Cost</span>
          <span class="odp-stat-value">{formatCost(liveCost)}</span>
        </div>
        <div class="odp-stat-divider"></div>
        <div class="odp-stat">
          <span class="odp-stat-label">Sessions</span>
          <span class="odp-stat-value">{agentSessions.length}</span>
        </div>
      </div>
    </div>
    ...
    <div class="odp-section odp-section--flex1">
      ...
      {#if agentActivity.length > 0}
        <div class="odp-activity-list">
          {#each agentActivity as evt, i (evt.id ?? i)}
            <div class="odp-activity-item">
              <span class="odp-activity-dot" style="background:{evt.level === 'error' ? 'var(--accent-error)' : evt.level === 'warning' ? 'var(--accent-warning)' : 'var(--accent-primary)'}"></span>
              <span class="odp-activity-text">{evt.message ?? evt.event_type ?? 'Activity'}</span>
            </div>
          {/each}
        </div>
      {:else}
        <div class="odp-activity-placeholder" role="status">
          ...
          <span class="odp-activity-text">No recent activity</span>
        </div>
      {/if}
    </div>
...
<style>
  ...
  .odp-activity-list {
    display: flex; flex-direction: column; gap: 6px; padding: 8px 16px; overflow-y: auto;
  }
  .odp-activity-item {
