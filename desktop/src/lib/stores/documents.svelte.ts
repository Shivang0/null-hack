...
class DocumentsStore {
  ...
  async selectDocument(doc: Document | null): Promise<void> {
    if (!doc) {
      this.selected = null;
      return;
    }
    ...
    if (!doc.content) {
      try {
        const full = await documentsApi.get(doc.path);
        const merged = { ...doc, ...full, content: full.content ?? '' };
        this.selected = merged;
        // Update in list too
        this.documents = this.documents.map(d => d.path === doc.path ? merged : d);
      } catch {
        this.selected = doc;
      }
    } else {
