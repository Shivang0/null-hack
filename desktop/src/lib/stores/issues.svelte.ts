...
class IssuesStore {
  ...
  async changeStatus(id: string, status: IssueStatus): Promise<void> {
    ...
    const updated = await this.updateIssue(id, { status });

    // Auto-dispatch agent when moved to todo or in_progress
    if (updated && issue?.assignee_id && (status === 'todo' || status === 'in_progress')) {
      // Small delay to let the backend process the status change
