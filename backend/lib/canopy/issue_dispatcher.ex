defmodule Canopy.IssueDispatcher do
  ...
  # Accept any agent state — auto-wake sleeping, queue for running/working
  defp validate_agent(%Agent{status: status}) when status in ["idle", "active"], do: :ok
  defp validate_agent(%Agent{status: "sleeping"} = agent) do
    agent |> Ecto.Changeset.change(status: "idle") |> Canopy.Repo.update!()
    :ok
  end
  defp validate_agent(%Agent{status: status}) when status in ["running", "working"] do
    # Agent is busy but we still allow dispatch — heartbeat will queue
    :ok
  end
  defp validate_agent(%Agent{status: _status}), do: :ok
end
