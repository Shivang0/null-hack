defmodule Canopy.Adapters.ClaudeCode do
  ...
  # The "result" event contains the final summary at top-level "usage".
  # Intermediate "assistant" events have usage nested under "message" -> "usage".
  defp extract_usage(%{"usage" => usage}) when is_map(usage) do
    extract_usage_from_map(usage)
  end
  ...
  # Result events also have modelUsage with costUSD — extract from there as fallback
  defp extract_usage(%{"modelUsage" => model_usage}) when is_map(model_usage) do
    # Pick the first (and usually only) model entry
    case Map.values(model_usage) do
      [usage | _] when is_map(usage) ->
        input = (usage["inputTokens"] || 0) + (usage["cacheReadInputTokens"] || 0) +
  ...
  defp extract_usage_from_map(usage) do
    input =
