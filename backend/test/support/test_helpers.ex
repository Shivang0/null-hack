defmodule Canopy.TestHelpers do
  ...
  alias Canopy.Repo
  alias Canopy.Schemas.{User, Workspace, Agent, BudgetPolicy, CostEvent, Approval, Team, TeamMembership, Department, Division, Organization, Engagement}
  ...
  def insert_engagement(workspace, attrs \\ %{}) do
    default = %{
      client_name: "Test Client #{unique_id()}",
      client_email: "client-#{unique_id()}@example.com",
    ...
    %Engagement{}
    |> Engagement.changeset(Map.merge(default, attrs))
    |> Repo.insert()
  end
  # ── Helpers ───────────────────────────────────────────────────────────────────
